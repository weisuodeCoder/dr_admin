<template>
  <Dialog :title="title" ref="show" width="800" :info="info">
    <template #body>
      <el-form :model="info.val" label-width="100" label-position="left" :rules="rules" ref="from">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="字典项编码" prop="code">
              <el-input-number :min="0" :disabled="disabled" v-model="info.val.code" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item :disabled="disabled" label="字典项名称" prop="label">
              <el-input :disabled="disabled" v-model="info.val.label" placeholder="请输入字典项名称" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="字典项排序" prop="sort">
              <el-input-number :min="0" :disabled="disabled" v-model="info.val.sort" />
            </el-form-item>
          </el-col>
          <el-col v-if="dict_data.val.type=='tree'" :span="12">
...
import { apiCreate, apiUpdate, apiShow } from '@/api/core/sso/dict-item';
...
const onShow = (_id, _type) => {
  ...
  title.value = '字典项【' + dict_data.val.name + '】' + join;
  onRefresh();
  show.value.onShow();
};
...
const doSubmit = (rule) => {
  ...
  rule.validate((valid) => {
    if (valid) {
      if (type == 'create') {
        // create
        info.val.dictId = dict_data.val.id;
