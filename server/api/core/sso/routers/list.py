...
def handle(input, jdbcTemplate):
    ...
    if(input.get("getChild")!=None and input.get("getChild")!=0):
        ...
        condition += " or router like '%" +str(input.get("getChild"))+ "%' "
    ...
    sql = "select * from sys_router where 1=1 "
    ...
    sqlEntry = SQLTool.getTotalParams("select count(1) from sys_router where 1=1 " + condition, params)
