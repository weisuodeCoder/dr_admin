...
def handle(input, jdbcTemplate):
    ...
    if(search.get("pid")!=None and search.get("pid")!=0):
        condition += " and dictId = ? "
        params.add(search.get("pid"))
    else:
        return Result(u"请选择字典！")
    if(search.get("name") != None and search.get("name")!=""):
        condition += " and name like '%"+search.get("name")+"%' "
    condition += " order by sort , createdTime "
    
    
    sql = "select * from dict_item where 1=1 "
    ...
    sqlEntry = SQLTool.getTotalParams("select count(1) from dict_item where 1=1 " + condition, params)
